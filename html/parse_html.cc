...
#include "mldb/ext/hubbub/parser.h"
      ...
      const FunctionContext & context) const
{
    ...
    vector<tuple<ColumnName, CellValue, Date> > vals;
    for (auto & l: parser.links) {
        vals.emplace_back(ColumnName("link"), CellValue(std::move(l)), text.getEffectiveTimestamp());
    }

    result.set("output", ExpressionValue(std::move(vals)));
